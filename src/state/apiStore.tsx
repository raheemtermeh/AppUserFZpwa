...
function reducer(state: State, action: Action): State {
  switch (action.type) {
    ...
    case "toggle_social_hub": {
      ...
      return {
        ...
        filters: {
          ...state.filters,
          socialHubIds: exists
            ? state.filters.socialHubIds.filter(
                (id) => id !== action.socialHubId
              )
    ...
    case "set_max_price":
      return {
        ...state,
        filters: { ...state.filters, maxPrice: action.value },
      };
    case "set_date":
    ...
    case "set_min_capacity":
      return {
        ...state,
        filters: { ...state.filters, minCapacity: action.value },
      };
    case "clear_filters":
      return {
        ...state,
      ...
      if (accessToken && refreshToken) {
        try {
          ...
          if (userData.user_type === "customer") {
            ...
            try {
              const reservationsData = (await apiService.reservations.list({
              ...
              console.error(
                "Failed to load reservations during auth restore:",
                error
              );
            }
          }
    ...
    const loadInitialData = async () => {
      ...
      try {
        ...
        try {
          ...
          const sortedFallbackHubs = (socialHubs as any).sort(
            (a: any, b: any) => {
              const ratingA = a.average_rating || 0;
              const ratingB = b.average_rating || 0;
              return ratingB - ratingA;
          ...
          dispatch({
            type: "set_error",
            key: "socialHubs",
            value: "Failed to load social hubs",
          });
        }
      }
  ...
  return useMemo(() => {
    ...
    return state.events.filter((e) => {
      ...
      if (
        ...
        !filters.categories.some((cat) => cat.id === e.category?.id)
      )
        return false;
      if (
        filters.socialHubIds.length &&
        !filters.socialHubIds.includes(e.social_hub?.id)
      )
        return false;
      if (filters.maxPrice != null && e.price > filters.maxPrice) return false;
      if (filters.date && e.date !== filters.date) return false;
      if (filters.rating != null && (e.average_rating || 0) < filters.rating)
        return false;
...
export function useFavoriteSocialHubs() {
  ...
  return useMemo(() => {
    if (
      !state.socialHubs ||
      !Array.isArray(state.socialHubs) ||
      !state.favorites ||
      !Array.isArray(state.favorites)
    )
      return [];
    return state.socialHubs.filter(
      (hub) => hub && state.favorites.includes(hub.id)
    );
...
export function useUserReservations() {
  ...
  return useMemo(() => {
    ...
    return state.reservations.filter((r) => {
      ...
      const customerId =
        typeof r.customer === "string" ? r.customer : r.customer?.id;
  ...
  useEffect(() => {
    ...
    pendingReservations.forEach((reservation) => {
      ...
      if (timeElapsed >= tenMinutes) {
        ...
        dispatch({
          type: "expire_pending_reservation",
  ...
  useEffect(() => {
    ...
    if (pendingReservations.length > 0 && !state.notification.show) {
      dispatch({
        type: "show_notification",
        message:
          "You have pending reservations. If you don't pay within 10 minutes, your tickets will be released.",
...
export function useApiActions() {
  ...
  const createReservation = async (eventId: string, numberOfPeople: number) => {
    try {
      ...
      const newReservation = await apiService.reservations.create(
  ...
  const removeFavorite = async (socialHubId: string) => {
    try {
      ...
      await apiService.customers.removeFavorite(
        state.auth.user.id,
        socialHubId
      );
  ...
  const createSocialHub = async (data: any) => {
    try {
      ...
      dispatch({
        type: "set_error",
        key: "socialHubs",
        value: "Failed to create social hub",
      });
      throw error;
    } finally {
      dispatch({ type: "set_loading", key: "socialHubs", value: false });
  ...
  const updateSocialHub = async (id: string, data: any) => {
    try {
      ...
      dispatch({
        type: "set_error",
        key: "socialHubs",
        value: "Failed to update social hub",
      });
      throw error;
    } finally {
      dispatch({ type: "set_loading", key: "socialHubs", value: false });
  ...
  const deleteSocialHub = async (id: string) => {
    try {
      ...
      dispatch({
        type: "set_error",
        key: "socialHubs",
        value: "Failed to delete social hub",
      });
      throw error;
    } finally {
      dispatch({ type: "set_loading", key: "socialHubs", value: false });
