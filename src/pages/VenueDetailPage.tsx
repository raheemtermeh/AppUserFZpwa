...
import {
  filterEventsWithOpenTicketSales,
  isTicketSalesClosed,
  shouldShowEventAsCompleted,
  shouldShowEventAsCancelled,
} from "../utils/eventStatusUpdater";
...
export default function VenueDetailPage() {
  ...
  const hubEvents =
    venueEvents.length > 0
      ? venueEvents
      : state.events.filter((e) => e.social_hub?.id === id);
  ...
  const getAmenityDisplayData = (amenityKey: string) => {
    const amenityMap: Record<string, { icon: string; name: string }> = {
      ...
      parking: {
        icon: "car",
        name: t("pages.venueDetail.amenityLabels.parking"),
      },
      foodService: {
        icon: "coffee",
        name: t("pages.venueDetail.amenityLabels.foodService"),
      },
      airConditioning: {
        icon: "snowflake",
        name: t("pages.venueDetail.amenityLabels.airConditioning"),
      },
      tvScreens: {
        icon: "tv",
        name: t("pages.venueDetail.amenityLabels.tvScreens"),
      },
      gamingEquipment: {
        icon: "gamepad-2",
        name: t("pages.venueDetail.amenityLabels.gamingEquipment"),
      },
      soundSystem: {
        icon: "volume-2",
        name: t("pages.venueDetail.amenityLabels.soundSystem"),
      },
      projector: {
        icon: "monitor",
        name: t("pages.venueDetail.amenityLabels.projector"),
      },
      restrooms: {
        icon: "bath",
        name: t("pages.venueDetail.amenityLabels.restrooms"),
      },
      outdoorSpace: {
        icon: "tree-pine",
        name: t("pages.venueDetail.amenityLabels.outdoorSpace"),
      },
      library: {
        icon: "book",
        name: t("pages.venueDetail.amenityLabels.library"),
      },
      smokingArea: {
        icon: "smoking",
        name: t("pages.venueDetail.amenityLabels.smokingArea"),
      },
    };
    ...
    return (
      <div className="text-center p-8">
        ...
        <h2 className="text-xl font-semibold text-slate-400 mb-2">
          {t("pages.venueDetail.loadingVenue")}
        </h2>
        <p className="text-slate-500">
          {t("pages.venueDetail.fetchingVenueDetails")}
        </p>
      </div>
    );
  }
  ...
  if (!hub) {
    return (
      <div className="text-center p-8">
        ...
        />
        <h2 className="text-xl font-semibold text-slate-400 mb-2">
          {t("pages.venueDetail.venueNotFound")}
        </h2>
        <p className="text-slate-500">
          {t("pages.venueDetail.venueNotFoundDescription")}
        </p>
      </div>
    );
  }
  ...
  // If redirected here with ?do=favorite after login, toggle favorite automatically
  const [searchParams] = useSearchParams();
  useEffect(() => {
    const doParam = searchParams.get("do");
    if (doParam === "favorite" && hub && state.auth?.isLoggedIn) {
      // call toggle after slight delay to ensure hub is present
      handleToggleFavorite();
      // remove the query param from URL (replace)
      navigate(`/venues/${id}`, { replace: true });
    }
  }, [searchParams, hub, state.auth?.isLoggedIn]);
  ...
  const pastEvents = [
    ...
    ...hubEvents.filter((event) => {
      ...
      const isAlreadyInPastEvents = hubPastEvents.some(
        (pastEvent) => pastEvent.id === event.id
      );
      return (
        !isAlreadyInPastEvents &&
        isTicketSalesClosed(event) &&
    ...
    <div className="space-y-6 md:space-y-8">
        ...
        <div className="relative">
          ...
          <div className="absolute top-4 left-4 right-4">
                  ...
                  <div className="flex items-center gap-1 bg-black/60 px-2 py-1 rounded-full text-xs">
                    <span className="text-white">
                      {hub.average_rating && hub.average_rating > 0
                        ? formatPersianNumber(hub.average_rating, 1)
                        : ""}
                    </span>
                  </div>
                  <div className="flex items-center gap-1 bg-black/60 px-2 py-1 rounded-full text-xs">
                    ...
                    <span className="text-white">
                      {formatPersianNumber(hub.events_count)}{" "}
                      {t("common.events")}
                    </span>
                  </div>
                  <button
            ...
            <div className="flex items-start justify-end">
              <div className="flex gap-2">{/* Rating button removed */}</div>
            </div>
          </div>
      ...
      <div className="card p-4 md:p-6">
        <div className="mb-6 bg-slate-800/40 p-1 rounded-xl relative">
          ...
          <div className="flex gap-1 overflow-x-auto scrollbar-hide pb-1 scroll-smooth">
            {[
              {
                key: "events",
                label: t("pages.venueDetail.tabs.events"),
                icon: "calendar",
              },
              {
                key: "reviews",
                label: t("pages.venueDetail.tabs.reviews"),
                icon: "star",
              },
              {
                key: "photos",
                label: t("pages.venueDetail.tabs.photos"),
                icon: "user",
              },
              {
                key: "info",
                label: t("pages.venueDetail.tabs.info"),
                icon: "location",
              },
            ].map((tab) => (
              <button
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-responsive-lg font-semibold">
                    {t("pages.venueDetail.chooseEventCategory")}
                  </h3>
                  <span className="text-responsive-sm text-slate-400">
                    {availableCategories.length}{" "}
                    {t("pages.venueDetail.categories")}
                  </span>
                </div>

                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                  {availableCategories.map((category) => {
                    const categoryEventsCount = hubEvents.filter(
                      ...
                      >
                        <div className="w-12 h-12 mx-auto mb-3 flex items-center justify-center">
                          <img
                            ...
                            alt={
                              t(`common.categoryNames.${category.name}`) ||
                              category.name
                            }
                        ...
                        <p className="text-xs text-slate-400">
                          {formatPersianNumber(categoryEventsCount)}{" "}
                ...
                {currentEvents.length > 0 && (
                  <div className="pt-6 border-t border-slate-700/50">
                    <div className="flex items-center justify-between mb-4">
                      <h3 className="text-responsive-lg font-semibold">
                        {t("pages.venueDetail.recentEvents")}
                      </h3>
                      <span className="text-responsive-sm text-slate-400">
                        {currentEvents.length} {t("pages.venueDetail.events")}
                      </span>
                    </div>
                    <div className="flex gap-3 md:gap-4 overflow-x-auto pb-4 snap-x snap-mandatory [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                      {currentEvents.slice(0, 5).map((event) => (
                        <div
                          key={event.id}
                          className="min-w-[240px] sm:min-w-[280px] md:min-w-[320px] lg:min-w-[360px] snap-start flex-shrink-0"
                        >
                          <EventCard e={event} />
                        </div>
                      ))}
              ...
              <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-responsive-sm text-slate-400">
                        {filteredCurrentEvents.length}{" "}
                        {t("pages.venueDetail.upcomingEvents")}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                {filteredCurrentEvents.length === 0 ? (
                  <div className="text-center py-8">
                    ...
                    <p className="text-slate-400">
                      {t("pages.venueDetail.noUpcomingEventsInCategory")}
                    </p>
                  </div>
                ) : (
                  <div className="flex gap-3 md:gap-4 overflow-x-auto pb-4 snap-x snap-mandatory [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                ...
                {filteredPastEvents.length > 0 && (
                  <div className="pt-6 border-t border-slate-700/50">
                    <div className="flex items-center justify-between mb-4">
                      <h3 className="text-responsive-lg font-semibold">
                        {t("pages.venueDetail.pastEvents")}
                      </h3>
                      <span className="text-responsive-sm text-slate-400">
        ...
        {activeTab === "reviews" && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-responsive-lg font-semibold">
                {t("pages.venueDetail.reviewsAndRatings")}
              </h3>
        ...
        {activeTab === "photos" && (
          <div className="space-y-4">
            <h3 className="text-responsive-lg font-semibold">
              {t("pages.venueDetail.photoGallery")}
            </h3>
              ...
              <div className="text-center py-8">
                <div className="text-slate-400 mb-2">
                  ...
                  <p className="text-responsive-base">
                    {t("pages.venueDetail.noPhotosAvailable")}
                  </p>
                </div>
              </div>
            )}
        ...
        {activeTab === "info" && (
          <div className="space-y-6">
            <h3 className="text-responsive-lg font-semibold">
              {t("pages.venueDetail.venueInformation")}
            </h3>
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between p-3 bg-slate-800/40 rounded-xl">
                <span className="text-responsive-sm text-slate-400">
                  {t("pages.venueDetail.address")}
                </span>
                <span className="text-responsive-sm font-medium text-right max-w-[60%]">
                  {hub.address}
                </span>
              </div>
              ...
              <div className="flex items-center justify-between p-3 bg-slate-800/40 rounded-xl">
                <span className="text-responsive-sm text-slate-400">
                  {t("pages.venueDetail.totalEvents")}
                </span>
                <span className="text-responsive-sm font-medium">
                  {formatNumber(hub.events_count, language)}
                </span>
              </div>

              <div className="flex items-center justify-between p-3 bg-slate-800/40 rounded-xl">
                <span className="text-responsive-sm text-slate-400">
                  {t("pages.venueDetail.averageRating")}
                </span>
            ...
            {/* Owner Info */}
            <div>
              <h4 className="text-responsive-base font-semibold mb-3">
                {t("pages.venueDetail.owner")}
              </h4>
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-4 mt-1 text-xs text-slate-400">
                    <div className="flex items-center gap-1">
                      ...
                      <span>
                        {t("pages.venueDetail.established")}{" "}
                        {hub.owner?.created_at
                          ? formatSolarHijriDate(hub.owner.created_at, "YYYY")
                          : "۱۴۰۰"}
                      </span>
                    </div>
                    <div className="flex items-center gap-1">
            ...
            {hub.description && (
              <div>
                <h4 className="text-responsive-base font-semibold mb-3">
                  {t("pages.venueDetail.aboutThisVenue")}
                </h4>
            ...
            <div>
              ...
              {venueAmenities.length > 0 ? (
                ...
                <div className="text-center py-6">
                  ...
                  />
                  <p className="text-slate-400">
                    {t("pages.venueDetail.noAmenitiesAvailable")}
                  </p>
                </div>
              )}
            </div>
                ...
                {hub.latitude && hub.longitude ? (
                  ...
                  <div className="w-full h-48 bg-slate-800/40 rounded-lg flex items-center justify-center">
                    <div className="text-center">
                      ...
                      />
                      <p className="text-slate-400">
                        {t("pages.venueDetail.locationNotAvailable")}
                      </p>
                    </div>
                  </div>
                )}
      ...
      <div className="flex gap-3">
        ...
        >
          ...
          <span className="ml-2">
            {isFavorite
              ? t("pages.venueDetail.removeFromFavorites")
              : t("pages.venueDetail.addToFavorites")}
          </span>
        </button>
      </div>
      {/* Rating Modal */}
      ...
      {showFullScreenPhoto && (
        ...
        >
          <button
            ...
            } w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 text-white grid place-items-center`}
          >
